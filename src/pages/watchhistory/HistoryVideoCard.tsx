    ...
    <Wrapper>
      ...
      <div className="video-info-container space-y-2">
        ...
        <div onClick={openChannel} className="flex items-center space-x-2 text-sm">
          {!hidesAvatar && <Avatar src={video.channelAvatar || defaultAvatar} alt="Channel avatar" />}
          {!hidesChannelName && <span className="secondary hover:text-gray-200">{video.channelName}</span>}
        </div>
        <p className="secondary">
          <span>Watched {timeSince(video.createdAt)} ago</span>
        </p>
      </div>
    </Wrapper>
  );
